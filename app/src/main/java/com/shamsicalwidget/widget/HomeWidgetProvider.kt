...
class HomeWidgetProvider : AppWidgetProvider() {
    ...
    override fun onDisabled(context: Context) {
        ...
        val appWidgetManager = AppWidgetManager.getInstance(context)
        val hasTextWidget = appWidgetManager.getAppWidgetIds(
            android.content.ComponentName(context, HomeTextWidgetProvider::class.java)
        ).isNotEmpty()
        val hasLockWidget = appWidgetManager.getAppWidgetIds(
            android.content.ComponentName(context, LockScreenWidgetProvider::class.java)
        ).isNotEmpty()
        if (!hasTextWidget && !hasLockWidget) {
